...
class Model_preparation(object):
    ...
    def __init__(self, args, gv):
        ...
        self.outputDir = gv.outputDir
        self.gene_ofInterest = gene_ofInterest
        file_handler = logging.FileHandler(os.path.join(self.outputDir, self.gene_ofInterest + '.log'))
        ...
        self.to_seed = args.to_seed
        if (self.to_seed):
            self.seed_num = 4
        self.enforce_corr_thresholding = args.enforce_corr_thresholding
        self.frac_test = args.frac_test  # fraction of cell lines to be used in the test set

        self.take_log2_tpm_forNNwTFs = gv.take_log2_tpm_forNNwTFs
        self.df_gencode_all_mrnas = gv.df_gencode_all_mrnas
        self.df_allgenes_tpm_anno = gv.df_allgenes_tpm_anno
        self.inputDir = gv.inputDir

        self.use_random_wts = gv.use_random_wts
        self.use_corr_wts = gv.use_corr_wts
        self.use_expDecay_wts = gv.use_expDecay_wts
        assert sum([self.use_random_wts, self.use_corr_wts, self.use_expDecay_wts]) == 1

        self.take_top_fts = gv.take_top_fts
        self.take_this_many_top_fts = gv.take_this_many_top_fts
        self.tf_corr_threshold_high = gv.tf_corr_threshold_high  # upper threshold for the TF corr; only TF with corr in between this and self.tf_corr_threshold_low are selected
        self.tf_corr_threshold_low = gv.tf_corr_threshold_low  # lower threshold for the TF corr
        self.get_random_TFs = gv.get_random_TFs  # default is False
        ...
        self.df_dhss_in_roi = copy.deepcopy(gv.df_dhss_in_roi)
        ...
        self.gene_ofInterest_info = gv.gene_ofInterest_info
