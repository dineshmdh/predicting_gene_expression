...
class Global_Vars(object):
    ...
    def __init__(self, gene_ofInterest, dist_lim=200,
                 tf_corr_threshold_high=1.0, tf_corr_threshold_low=0.3, use_tad_info=True,
                 take_top_fts=True, take_this_many_top_fts=20,
                 take_log2_tpm=True, use_wts="corr",
                 use_random_DHSs=False, use_random_TFs=False, outputDir=os.getcwd()):
        '''Defining some key parmeters first followed by less important parameters

        Arguments that are not obvious from their names:
        dist_lim -- distance (in kb) from TSS to go and collect DHS sites for the model.
                    (Note that it is the same distance upstream and downstream)
        tf_corr_threshold[_high/_low] -- pearson corrlelation values between CellNet TFs and self.
                    gene_ofInterest in between these thresholds are selected for the model.
                    If there is no such cellNet TF, two top ranking TFs (given PCC > 0.3)
                    are selected. In this script, this is only used to write output file suffix.
        take_top_fts -- only pertains to DHS sites. If True, only self.take_this_many_top_fts
                    top DHS sites ranked by PCC with self.gene_ofInterest expression profile
                    are selected. If False, all DHS sites are used. If the total number of
                    DHS sites for this gene_ofInterest is < self.take_this_many_top_fts, then
                    all DHS sites will be used. This helps in limiting the number of parameters
                    to learn and from dying on its own sometimes because of excessive memory
                    consumption.
        take_log2_tpm -- if True, the TPM values for this gene_ofInterest are log2 transformed
                    before being standardized (for trainY/testY), i.e. x = np.log2(x+1). In this
                    script, this is used only in generating the output file suffix. This variable
                    should be the same as Model_preparation variable take_log2_tpm_forNNwTFs.
        use_wts -- can be one of "corr", "random", or "expDecay". Determines the weights used in
                    initialization of the neural net. This is onl used in generating the output
                    file suffix.
        ...
        self.use_random_DHSs = use_random_DHSs  # if True, random region of interest (roi) is selected for the model
        self.use_random_TFs = use_random_TFs  # if True, random TFs are used for self.gene_ofInterest
        self.outputDir = outputDir  # generated using  get_output_dir from helper_functions
        self.out_suffix = self.get_out_suffix()
    ...
    def __repr__(self):
        out_info = "\nGeneral Info: " + self.gene_ofInterest + self.out_suffix
        if (self.use_tad_info):
